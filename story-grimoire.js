// story-grimoire.js — 「分類魔導書」版劇情腳本（版權安全備用；預設用這版）
//
// 這是分類帽版（story.js）的換殼版：語氣、笑點、化學全部保留，只把「帽」的
// 自稱／道具（本帽、帽簷…）換成「書」的意象（本書、老夫、書角…）。
// 角色圖用 assets/images/character/magicbook.webp。
//
// 格式與 story.js 完全相同：{ who: "hat" | "wiz", text, expr? }
//   who 仍沿用 "hat" 當「導師角色」的內部鍵（不代表帽子），故事選擇由角色開關決定。
// ★ 改台詞只動引號內文字。★

const StoryScriptsGrimoire = {

  // Level 1：碳氫骨架（烷 / 烯 / 炔 / 芳香）
  level1: [
    { who: "hat", expr: "annoyed",   text: "……又一個新面孔。{name}，對吧。本書扉頁記過的學生，比你吃過的米還多。坐下，別折我的書角。" },
    { who: "wiz", text: "分、魔導書前輩好！可是這些分子全是碳跟氫，長得……根本一模一樣吧？" },
    ...
    { who: "hat", expr: "neutral",   text: "……勉強算孺子可教。記住，碳氫骨架是地基；地基看歪，後面掛再多官能基都是危樓。滾去做題——老夫的時間比你的成績值錢。" }
  ],
  ...
  level3: [
    ...
    { who: "hat", expr: "annoyed",   text: "想太多。我只是在核對事實——本書從不誇人，只確認真相。去做題，{name}。" }
  ],
  ...
  // Level 6：終極分類（Level 1~5 綜合題）
  level6: [
    { who: "hat", expr: "neutral",   text: "烷烯炔芳香、醇醚、醛酮、羧酸酯、胺鹵化物——全員到齊。這一關，我把它們全倒進同一鍋。" },
    { who: "wiz", text: "全、全部混在一起？！等等，你之前好像說過……酚是自己一類？" },
    { who: "hat", expr: "annoyed",   text: "記性不錯，難得。–OH 直接黏在苯環的碳上，那是『酚』，自立門戶。以為掛了苯環的 –OH 還能算醇？那是懶人的想法，而懶人在老夫這幾頁一律不及格。" },
  ...
  levelShell: [
    ...
    { who: "hat", expr: "thinking",  text: "酮、醚、鹵化物、胺，一個都跑不掉。至於『芳香烴』——得整隻乾乾淨淨，只有龜殼加烷基尾巴，像甲苯、二甲苯那種，才輪得到它。教你一招老夫壓箱底的：伸手把龜殼遮住，剩下那一小塊，就是答案。" },
  ...
  // Level 99：資優全英挑戰（選做）
  level99: [
    { who: "hat", expr: "annoyed",   text: "……又是你。{name}，先確認一下：你知道這關的選項全是英文吧？現在退場還來得及，老夫不會笑你——好吧，會笑一下。" },
...
// === Family-keyed aliases（與 story.js 對齊）===
StoryScriptsGrimoire.hydrocarbon    = StoryScriptsGrimoire.level1 || [];
StoryScriptsGrimoire.oxygen         = StoryScriptsGrimoire.level2 || StoryScriptsGrimoire.level3 || StoryScriptsGrimoire.level4 || [];
StoryScriptsGrimoire.nitrogenHalide = StoryScriptsGrimoire.level5 || [];
StoryScriptsGrimoire.mixed          = StoryScriptsGrimoire.level6 || [];
StoryScriptsGrimoire.shell          = StoryScriptsGrimoire.levelShell || [];
