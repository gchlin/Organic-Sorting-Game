...
- **Practice**：不倒數。每輪 10 題，做完後結算；正確率達 70% 視為通關。結算後可選擇繼續練習剩餘未出題目，並混入本輪答錯題目，或離開關卡。
...
- Practice 每輪 10 題；完成後若正確率達 70%，該關標記為通關。
- Practice 結算後，玩家可選擇繼續練習：題池包含尚未出現的題目，並加入本輪答錯題目。
- Practice 的關卡通過率需記錄，並在圖鑑中可查看。
- Speed 不計入圖鑑解鎖，不解鎖劇情。
...
| Practice 每輪題數 | 10 題 |
| Practice 通過正確率 | 70% |
| Practice 解鎖劇情門檻 | 完成 10 題且正確率達 70% |
| Speed 初始時間 | 1 分鐘 |
