...
- ✅ **新關卡「龜殼與它的產地」（苯環陷阱關）**：用現有苯環衍生物組題（苯酚 / 苯甲醇 / 苯甲醛 / 苯甲酸 / 甲苯·二甲苯·苯乙烯 / 苯乙酮 / 苯甲酸甲酯 / 苯甲醚 / 苯胺 / 氯苯 / 萘——全部已有 SVG），干擾項刻意排段考陷阱組（酚 ↔ 苯甲醇、苯甲醛 ↔ 苯甲酸）。零架構成本，純加題庫 + 一個關卡入口。
...
**待辦**
- [ ] 「其他」桶要收哪些分子的最終清單（草案見下節 / `化學審查清單.md`）→ 確定後請 Haiku 用 `assets/images/generate_functional_groups.py` 那套 RDKit 流程生 SVG。
- [ ] NotebookLM 整理「課本式」的：13 類「為什麼」辨識提示、每關新手教學 2–4 頁文案、各關劇情擴寫。
- [ ] `game.js` modal 擴充支援圖片 + 每關教學投影片資料。
- [ ] 「龜殼與它的產地」題庫資料（`QuestionSets` 加一關）。
